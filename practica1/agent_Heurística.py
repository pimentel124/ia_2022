...
class Estat:
    ...
    def genera_fills(self, string: str):
        ...
        for i, j in enumerate(diccionari_moviments.values()):
            coordenades = {sum(location) for location in zip(self.__pos_agent[string], j)}
...
class Rana(joc.Rana):
    ...
    def cerca(self, estat:Estat, string:str):
        ...
        self.__oberts.put((estat.calc_heuristica(string), estat))
        ...
        while not self.__oberts.empty():
            if actual.es_meta(string):
                ...
                while iterator.pare is not None:
                    pare, accio = iterator.pare
                    acciones.append(accio)
                    iterator = pare
