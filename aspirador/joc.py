...
class Casa(joc.Joc):
    ...
    def _aplica(self, accio: super_entorn.Accio) -> None:
    ...
    def percepcio(self) -> super_entorn.Percepcio:
        return super_entorn.Percepcio(
            {
...
class Casal(joc.Joc):
    def __init__(self, agents: list[agent.Agent], mida_casa: tuple[int, int]):
        ...
        self.__habitacions = [[EstatHabitacio.aleatori() for _ in range(mida_casa[0])] for _ in
                              range(mida_casa[1])]
